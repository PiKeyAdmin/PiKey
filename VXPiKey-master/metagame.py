...
def randomcolor( targetbrightness = 100): 
    Rcolor = int( random() * 10 )
    Gcolor = int( random() * 10 )
    Bcolor = int( random() * 10 )
    norm = 5.0*targetbrightness / (Rcolor + Gcolor + Bcolor)
...
class GameChunkClass:
    ...
    def draw( self, screen ):
        ...
        #teken stuff... (niks als standaard)
    def quit( self ):
        pass
...
class PianoKeyClass( GameElementClass ):
    def __init__( self, **kwargs ):
        self.allowedchanges = [ "on", # set noten naar de velocity
                                "fadespeed", # hoe snel een noot uitgaat
                                "width",
                                "white",
                                "length",
                                "whitekeyindex",
        ...
        # set wat de gebruiker invoert
        self.setstate( **kwargs )
        self.fillcolor = (0,0,0) # later
